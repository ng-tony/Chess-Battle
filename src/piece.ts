...
export const validateMove = (from:number, to:number, squares:(Piece | null)[]):boolean => {
...
const removeInvalidMoves = (potentialMoves:number[], loc:number, squares:(Piece | null)[]) => {
...
const checkCheck = (color:Color, squares:(Piece | null)[]):boolean => {
    for (const [i, square] of squares.entries()){
        ...
        const checks = getCaptureMoves(i, squares).filter( (move) => {
            return (squares[move]?.color === color 
...
const bishopMoves = (loc:number, squares:(Piece | null)[]):number[] => {
    ...
    const operations = [(n:number) => loc+n*9, (n:number) => loc+n*-9,
                        (n:number) => loc+n*7, (n:number) => loc+n*-7];
...
const pawnMoves = (loc:number, squares:(Piece | null)[]):number[] => {
    let potential_moves = [];
    let [op, startingPos] = squares[loc]?.color === Color.white ? 
...
const queenMoves = (loc:number, squares:(Piece | null)[]):number[] => {
    ...
    const operations = [(n:number) => loc+n*9, (n:number) => loc+n*-9,
        (n:number) => loc+n*7, (n:number) => loc+n*-7,
        (n:number) => loc+n*8, (n:number) => loc+n*-8,
        (n:number) => loc+n*1, (n:number) => loc+n*-1];
...
const rookMoves = (loc:number, squares:(Piece | null)[]):number[] => {
    let potential_moves = [];
    const operations = [(n:number) => loc+n*8, (n:number) => loc+n*-8,
        (n:number) => loc+n*1, (n:number) => loc+n*-1];
    for (let op of operations){
        for (let i = 1; i < BOARD_SIZE; i++){
            potential_moves.push(op(i));
...
const pawnAttackMoves = (loc:number, squares:(Piece | null)[]):number[] => {
    let op = squares[loc]?.color === Color.white ? 
        (b:number) => loc + b:
        (b:number) => loc - b;
...
const kingAttackMoves = (loc:number, squares:(Piece | null)[]):number[] => {
    return removeInvalidMoves([-9, -8, -7, -1, 1, 7, 8, 9].map((a) => a+loc), loc, squares)
}

const filterCaptureMoves = (strategy: (loc: number, squares: (Piece | null)[]) => number[]) => {
